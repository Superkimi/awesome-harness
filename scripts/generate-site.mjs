  ...
  const walk = (dir) => {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(full);
        ...
        if (!relative.startsWith(".git/") && ![".png", ".jpg", ".jpeg", ".gif", ".webp", ".mp4", ".mov", ".zip", ".lock"].includes(ext)) {
          try {
...
write(path.join(siteRoot, "assets/harness.css"), css + supplementalCss + richCss + matrixCss + legacyCss + detailCss + sourceMapCss + inventoryCss + conceptCss + lineAnnotCss);
