1. 同事说聊天发出后工具没有继续，我先沿着 SessionTurn 看一次运行的闸门。
2. 这一章不背概念，先看 DeepChat 怎样把聊天升级成可恢复的 Agent Harness。
3. 固定版本证据：src/main/session/turn.ts，只展示源码片段和中性文件名。
4. 实现事实一：sendMessageUnderSessionGate|startInitialTurn。
5. 实现事实二：er; signal?: AbortSignal } ): Promise<MessageStartResult> { return await this.dependencies.workdir.runWithSessionOperationGate( se。
...
11. turn boundary 和 session gate 让一次任务有明确的推进节奏。
