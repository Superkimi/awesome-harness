1. 昨天的对话今天还要继续，我先看看 DeepChat 如何保留上下文。
...
7. 源码里能看到：, SessionTurnRuntimePort, SessionTurnStorePort, SessionTurnTranscriptPort } from './contracts' export int。
8. 小白复现：先写目标，再列数据；每一步保留状态，最后再预览或交付。
9. 再看一个边界：功能面很宽，旧 runtime 与新 harness 并存时需要 cleanup guard
10. 所以它值得学的不是按钮，而是把过程变成可以检查的证据。
11. 今天记住：先证明一个真实工作结果，再追问它为什么能稳定完成：把 DeepChat 想成一台带录像机的桌面工作站：每次工具动作和会话分叉都留下可恢复的带子，而不是只保留最后一句话。
