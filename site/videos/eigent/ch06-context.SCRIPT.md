1. 客户一周的资料塞进来，我最怕 Agent 把项目记忆当成聊天废话。
2. 这一章不背名词，先看 Eigent 怎样把桌面任务拆成能交接的状态。
3. 固定版本证据：backend/app/service/single_agent_service.py，只展示源码片段和中性文件名。
4. 实现事实一：MEMORY_TOKEN_BUDGET|conversation_history|durable。
5. 实现事实二：import get_working_directory logger = logging.getLogger("single_agent_service") # Char budget for the durable memory bundle (~32k 。
...
11. 记忆需要 token budget、历史边界和可持久化的任务上下文。
