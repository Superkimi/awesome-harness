1. 老板让我接一个 MCP 工具，但权限和配置不能一股脑全开。
2. 这一章不背名词，先看 Eigent 怎样把桌面任务拆成能交接的状态。
3. 固定版本证据：backend/app/service/task.py，只展示源码片段和中性文件名。
4. 实现事实一：activate_toolkit|search_mcp|install_mcp。
5. 实现事实二：-> user deactivate_agent = "deactivate_agent" # backend -> user request_usage = "request_usage" # backend -> user assign_task = "a。
