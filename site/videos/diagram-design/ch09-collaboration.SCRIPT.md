1. 产品、研发、设计各自改图最容易互相覆盖，我先给 ownership、routing 和 escalation 设边界。
2. 这一章不背图表名词，先看 Diagram Design 怎样把语义、版式和检查写成可调用的 Skill。
3. 固定版本证据：skills/diagram-design/SKILL.md，只展示源码片段和中性文件名。
4. 实现事实一：ownership|routing|escalation。
5. 实现事实二：nt / scope | **Nested** | [type-nested.md](references/type-nested.md) | | Parent → children relationships | **Tree** | [type-tree.。
