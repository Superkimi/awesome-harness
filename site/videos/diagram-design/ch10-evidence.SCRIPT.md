1. 评审只问一句“这图能不能拿去讲”，我用 checklist、反模式和可访问性证据回答。
2. 这一章不背图表名词，先看 Diagram Design 怎样把语义、版式和检查写成可调用的 Skill。
3. 固定版本证据：skills/diagram-design/SKILL.md，只展示源码片段和中性文件名。
4. 实现事实一：Pre-Output Checklist|anti-pattern|accessibility。
5. 实现事实二：ba(45,49,66,0.12)` - `border-radius: 6px`, `padding: 1.25rem` - **No `box-shadow`** - Card dots: 7px, `border-radius: 50%` — ink /。
