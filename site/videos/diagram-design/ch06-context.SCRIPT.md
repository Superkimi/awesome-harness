1. 评审说图太挤，我不再缩小字体，而是用 complexity budget 决定拆成几张。
2. 这一章不背图表名词，先看 Diagram Design 怎样把语义、版式和检查写成可调用的 Skill。
3. 固定版本证据：skills/diagram-design/SKILL.md，只展示源码片段和中性文件名。
4. 实现事实一：complexity budget|split。
5. 实现事实二：ing, pick the table. - If two types seem useful, pick the dominant axis; a semantic pattern may add behavior-specific primitives, 。
