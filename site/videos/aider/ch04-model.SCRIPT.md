1. 客户临时换模型，业务流程不能跟着重写；我先看 LiteLLM 适配层怎样托住差异。
2. 这一章不猜作者意图，只沿着固定版本的运行时代码、协议和测试看事实。
3. 固定版本证据：aider/models.py:985-1037 · LiteLLM 是统一 Provider 适配层。
4. 事实一：Aider 把各家模型 API 的差异交给 LiteLLM，自己的核心只面对一套近似 OpenAI 的消息格式。
5. 源码含义：接模型很快，但 Provider 行为、重试和能力元数据也部分受 LiteLLM 语义约束。
6. 事实二：Aider 把各家模型 API 的差异交给 LiteLLM，自己的核心只面对一套近似 OpenAI 的消息格式。
