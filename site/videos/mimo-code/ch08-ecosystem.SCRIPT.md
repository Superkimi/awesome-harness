1. 老板要把 Skill 接进 Compose，我先确认插件入口和扩展边界。
2. 这一章不背术语，先看 MiMo-Code 怎样把内核、Compose 和插件能力分开。
3. 固定版本证据：packages/plugin/src/index.ts，只展示源码片段和中性文件名。
4. 实现事实一：plugin|extension。
5. 实现事实二：ter(type: string, adaptor: WorkspaceAdaptor): void } serverUrl: URL $: BunShell } export type PluginOptions = Record<string, unkno。
...
11. 插件是可注入能力，但必须经过清晰的 extension 入口。
